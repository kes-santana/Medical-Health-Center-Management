...
\begin{document}
	...
	El proyecto está desarrollado en su totalidad  en \textit{Python} (versión 3.13.2) y se recomienda el uso de un \textit{virtual environment}.
	
	\subsection{Requerimientos}
	Leer archivo \textit{requirements.txt} adjunto al archivo.
	
	Cumplidos estos requerimentos, para ejecutar el programa solo es necesario ejecutar el script \textit{main.py} el cual activará el virtual environment y ejecutará la app.
	
	\section{Diseño}
	
	Se usa \textbf{Clean Architecture} junto a principios \textbf{SOLID} para obtener mejor mantenimiento, escalabilidad y separar claramente las responsabilidades del código, para obtener de esta forma mayor vida útil del software, asegurando así, mayor facilidad para pruebas unitarias y añadir nuevas funcionalidades. Además se usa un patrón \textbf{MVVM} para separar la lógica del programa de la interfaz gráfica.
	
	La \textbf{experiencia de usuario y el diseño de interfaz de usuario (UX-UI)} se centra en dar una experiencia intuitiva del uso de las funciones implementadas  usando un diseño minimalista priorizando la accesibilidad y claridad de navegación junto a consistencia en los botones y retroalimentación visual inmediata. Además, se escoge una paleta de colores asociada a clínicas de salud haciendo un llamado a la limpieza y la higiene.
	...
	Se ha tenido en cuenta la posibilidad de programar citas para el primer espacio disponible en el día así como la posibilidad de programar una cita en el primer espacio libre en la agenda general de un doctor determinado.
	...
	\item Se implementó la posibilidad de asignar a cada empleado un nuevo papel en la clínica, dándole a estos mayores facultades en el sistema de administración.

	\item El sistema tiene la posibilidad de graficar la cantidad de citas diarias y la cantidad de pacientes que posee un empleado en un transcurso de tiempo determinado.
\end{itemize}
	
\section{Aprendizaje durante el desarrollo del proyecto}
...
En el backend se priorizó el uso de librerías que trabajan con el tiempo como \textit{Datetime} y \textit{Calendar} demandando el aprendizaje de estas para un mejor resultado, dado que esta es la problemática principal que se debe resolver. De igual forma, se utiliza \textit{plotly-express}, librería para el trabajo con gráficos y datos, siendo un aspecto a estudiar.

Para el frontend se utiliza la librería \textit{streamlit} la cual por su valor y complejidad fue de vital importacia la adaptación a su modo de uso para la obtención de una mejor calidad y una mejor experiencia de trabajo del usario.
...
\section{Modo de uso}
 ...
 Al momento de acceder al programa usted se encontrará con una página de inicio de sesión, que le mostrará un resumen de las consultas  de la clínica correspondientes al mes en curso. Para acceder por primera vez se ha creado el super usuario \textit{admin} y su contraseña es \textit{admin}.
 
Después de eso usted tendrá accesso al sistema como administrador y deberá crear su cuenta personal y, por seguridad cambiar la contraseña del administrador. Para esto dirijase a la sección \textit{Empleados} que se encuentra en la barra lateral de opciones y dirijase a la pestaña \textit{Crear}. Aquí deberá rellenar el formulario solicitado, y terminado esto, se creará su nueva cuenta. Se debe precisar que, el proceso es el mismo para agregar a otros empleados y solo lo podrá hacer si tiene acceso como administrador, además los usarios recién creados tendrán como nombre \textit{nuevo-empleado-nombre-del-empleado} en minúsculas y guión bajo, y como contraseña \textit{0123456789}. 
 ...
 En la sección \textit{Dashboard} encontrará la página principal del programa donde podrá observar variada información.
 ...
 Finalmente, en la sección \textit{System} podrá cambiar el tipo de acceso que tendrán los usuarios. Debe tener en cuenta que los usuarios con acceso de doctor solo podrán programar sus propias citas, los que tengan acceso de secretario podrán programar citas de cualquier doctor y los administradores tendrán acceso global al sistema.

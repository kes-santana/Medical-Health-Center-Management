...
from Frontend.app import ROL, USER
...
with user_container:
    ...
    with col1:
        st.write("")
        st.write("")
        st.image("Frontend/user_icon1.jpg",)
    ...
    with col2:
        ...
        st.write(f"ID: poner el id!!!!!".title())
...
with opcion_expander:
    ...
    with user_password_expander:
        ...
        with st.form("change_user_password_form"):
            ...
            user_id = st.number_input("User ID", min_value=1)
            new_password = st.text_input("New User password")
            password = st.text_input("User Password")
            ...
            if submitted_user_password:
                try:
                    cambiar_password_de_usuario(user_id, password, new_password)
